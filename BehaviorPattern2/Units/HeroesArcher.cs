...
namespace BehaviorPattern2
{
    class HeroesArcher : Unit, IArcher
    {
        ...
        public int Amunition => _ammunition;

        public HeroesArcher(IMap map, string name,int range, int damage, int amunition):base(map, name)
        {
            _range = range;
            _damage = damage;
            _ammunition = amunition;
            Comands.Add(new ShootComand(this));
        }
        ...
        {
            return visitor.VisitArcher(this);
        }
    }
}
